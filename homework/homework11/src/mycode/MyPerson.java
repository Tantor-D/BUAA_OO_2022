...
public class MyPerson implements Person {
    ...
    public void renewNextNodeForPrim(PriorityQueue<NextNode> nextNodes,
                                     HashSet<Integer> finishSet) {
    ...
    public void renewNextNodeForDijkstra(PriorityQueue<NextNode> nextNodes,
                                         HashSet<Integer> finishSet,
                                         int nowDis) {
        for (int personId : values.keySet()) {
            if (!finishSet.contains(personId)) {
                nextNodes.add(new NextNode(personId, nowDis + values.get(personId)));
            }
        }
    }
    ...
    public void clearNotices() {
        ...
        while (it.hasNext()) {
            Message thisMessage = it.next();
            if (thisMessage instanceof NoticeMessage) {
                it.remove();
            }
        }
    }
