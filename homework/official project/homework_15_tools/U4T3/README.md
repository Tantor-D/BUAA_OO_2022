...
学生需要实现一个自己的`MyImplementation`类，这个类必须继承接口`com.oocourse.uml3.interact.format.UserApi`。
...
public interface UserApi
        extends UmlClassModelApi, UmlStandardPreCheck,
        UmlCollaborationApi, UmlStateChartApi {
}
...
```bash
java -jar U4T3.jar dump -s "R001.mdj" -n Model1 -t UMLModel
```
...
- 此外，对于`ClassNotFoundException`（全称`com.oocourse.uml3.interact.exceptions.user.ClassNotFoundException`）等几个异常类，在 Java 的标准库里面有与之同名的类（全称`java.lang.ClassNotFoundException`）。**请各位在使用的时候注意甄别，以免误用**。
