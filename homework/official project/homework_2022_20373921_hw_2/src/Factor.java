...
public class Factor implements Comparable<Factor> {
    ...
    public Factor mergeTwoFac(Factor other) {
        // todo 写合并的函数，注意要新建空间
        // todo 还有一种思路，删掉列表中的一个sin^2,然后删掉另一个factor
    ...
    public void refineFac() { //这里的refine并不考虑对sin(常数)进行化简操作
        //看看有无可以sin(0)，直接清空这个Factor，todo 删除这个Factor的事交给Function
        ...
        while (hasFind) {
            ...
            for (TriFac tri : sinMap.keySet()) {
                if (!tri.getDd().equals(BigInteger.ZERO)) {
                    continue; // 不是常数，跳过
                } else { // 是常数，接着做  todo biginteger算完后返回的是一个新的值？？
