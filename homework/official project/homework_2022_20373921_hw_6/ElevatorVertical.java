public class ElevatorVertical extends Elevator {
    ...
    public ElevatorVertical(AskQueue askQueue, int building, int id) {
        this.id = id;
    ...
    public void run() {
        while (true) {
            ...
            if (isNeedOut(persons.getAsksUnsafe(), nowFloor)) { // 下人
            ...
            // 走不走
            if (!persons.isEmpty() || isRequestExitThisDirection()) {
                sleepForUnitTime();
                move();
    ...
    public boolean isNeedTurn() {
        if (numOfPeople > 0 || isPersonWantIn() || isRequestExitThisDirection()) {
            return false;
        }
        return true;
    }
    ...
    public boolean isRequestExitThisDirection() {
