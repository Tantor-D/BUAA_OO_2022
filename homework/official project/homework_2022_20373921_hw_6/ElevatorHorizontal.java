public class ElevatorHorizontal extends Elevator {
    ...
    public ElevatorHorizontal(AskQueue askQueue, int floor, int id) {
        this.id = id;
    ...
    public void run() {
        while (true) {
            ...
            // 开门下人
            if (isNeedOut(persons.getAsksUnsafe(), nowBuilding)) {
            ...
            if (numOfPeople > 0 || isRequestExitThisDirection()) {
                sleepForUnitTime();
                move();
    ...
    public boolean isNeedTurn() {
        ...
        if (isRequestExitThisDirection()) { // 前方有请求，那就不转向
            return false;
        }
