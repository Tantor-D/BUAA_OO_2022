...
public class Function { // todo 时刻考虑function为空的情况
    ...
    public boolean equalZero() { // todo 这个可能会根据状况修改，因为hashmap的key不可能为空
        return factors.isEmpty();
    }
    
    public void printFunc() {
        if (factors.isEmpty()) {
            System.out.print("0"); //todo 因为有function的嵌套，最后看一下会不会有bug
            return;
        }
        // todo 最后加一个优化，把正数的放前面
    ...
    public Function sinFunc() { //todo 看一下是不是每一种操作都考虑到了为空的情况
        Function newFunc = new Function();
    ...
    @Override
    public int hashCode() { //todo 这个一定要重写
